...
implementation
    ...
    var NumValue, StrValue,IDName   : String;
        ID1, ID2, ID3, ID4, ID5                    : PIdentifier;
    begin
        Result := nil;
        ...
        4:  Begin // ()
                ...
                begin
                    ...
                    Begin
                        ...
                        if ID1 <> nil then
                            if MyParser^.NextIs(')') <> '' then
                                Result := New(PSinIdentifier, Init('', ID1)) 
                            else
                                ErrorString := '")" expected' 
                        else
                            ErrorString := 'Argument expected; '+ErrorString;
                    End else ErrorString := '"(" expected';
                end else
                
                if MyParser^.NextIs('PSET') <> '' then
                begin
                    if MyParser^.NextIs('(') <> '' then
                    Begin
                        ID1 := ProcessWithRank(0);
                        if ID1 = nil then ErrorString := 'Argument expected; '+ErrorString else
                        if MyParser^.NextIs(',') = '' then ErrorString := '"," expected; '+ErrorString else
                        begin
                          ID2 := ProcessWithRank(0);
                          if ID2 = nil then ErrorString := 'Argument expected; '+ErrorString else
                          if MyParser^.NextIs(',') = '' then ErrorString := '"," expected; '+ErrorString else
                          begin
                            ID3 := ProcessWithRank(0);
                            if ID3 = nil then ErrorString := 'Argument expected; '+ErrorString else
                            if MyParser^.NextIs(',') = '' then ErrorString := '"," expected; '+ErrorString else
                            begin
                              ID4 := ProcessWithRank(0);
                              if ID4 = nil then ErrorString := 'Argument expected; '+ErrorString else
                              if MyParser^.NextIs(',') = '' then ErrorString := '"," expected; '+ErrorString else
                              begin
                                ID5 := ProcessWithRank(0);
                                if ID5 = nil then ErrorString := 'Argument expected; '+ErrorString else
                                if MyParser^.NextIs(')') = '' then ErrorString := '")" expected; '+ErrorString else
                                begin
                                  Result := New(PPsetIdentifier, Init('', ID1, ID2, ID3, ID4, ID5));
                                end;
                              end;
                            end;
                          end;
...
//                                Result := New(PPsetIdentifier, Init('', ID1, ID2, ID3, ID4, ID5)) else
//                                ErrorString := '")" expected' else
//                            ErrorString := 'Argument expected; '+ErrorString;
                    End else ErrorString := '"(" expected';
                end else
